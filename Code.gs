...
function getDashboardData(filters) {
  try {
    ...
    let stats = {
      ...
      hotspotDetails: [], 
      caseStatus: { finished: 0, prosecutorSubmitted: 0, courtResult: 0, investigating: 0, oyaFine: 0 }, 
      monthlyCounts: {},
    ...
    rows.forEach((row) => {
      ...
      if (caseResultText.includes("ส่งฟ้อง")) stats.caseStatus.prosecutorSubmitted++;
      if (caseResultText.includes("ผลคดีชั้นศาล")) stats.caseStatus.courtResult++;
...
function generateInsight(stats) {
  ...
  result.caseStatusSummary = { finished: stats.caseStatus.finished, oyaFine: stats.caseStatus.oyaFine, prosecutorSubmitted: stats.caseStatus.prosecutorSubmitted, courtResult: stats.caseStatus.courtResult, investigating: stats.caseStatus.investigating };
